...
class TourTeleopSession(Node):
    def __init__(self):
        ...
        self.get_logger().info(
            "Tour teleop session started for route '%s'. Type r then Enter to save, esc to exit."
            % self.route_label
        )
            ...
            while rclpy.ok():
                if self._key_ready():
                    ...
                    if event.key == "\x1b":
                        ...
                        break
                    if event.key in ("\r", "\n"):
                        self.handle_line(self.consume_line())
                        continue
                    if len(event.key) == 1 and event.key.lower() == "r":
                        self.line_buffer = ["r"]
                        self.get_logger().info("Save armed. Press Enter to publish /save_tour_command.")
                        continue
    ...
    def print_help(self) -> None:
        ...
        print("  r + Enter : save current tour to /save_tour_command")
